...
let myClubCache: Club | null | undefined;
let clubListCache: Club[] | undefined;

function getStoredMyClub(): Club | null {
  if (myClubCache === undefined) myClubCache = loadMyClubFromStorage();
  return myClubCache;
}
...
export default function ClubsPage() {
  ...
  // Mock user's club (null = not in a club) — read via external store so the
  // first client render matches SSR and localStorage writes stay observable.
  const storedMyClub = useSyncExternalStore(subscribeStoredClubs, getStoredMyClub, () => null);
  const storedList = useSyncExternalStore(subscribeStoredClubs, getStoredClubList, () => []);

  const myClub = storedMyClub;
  const clubList = useMemo(() => {
    const base = storedList.length > 0 ? storedList : topClubs;
    return storedMyClub && !base.some((c) => c.id === storedMyClub.id)
      ? [storedMyClub, ...base]
      : base;
  }, [storedMyClub, storedList]);
            ...
            <div className="flex flex-col divide-y">
              <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">0 trophies contributed</div>
                </div>
              </div>
